...
{
    SparseArrayCompat<ItemViewDelegate<T>> delegates = new SparseArrayCompat();  // 观察者集合
    ...
    public int getItemViewType(T item, int position)
    {
        ...
        {   // 早到符合这个位子的控件
            ...
            {
                return delegates.keyAt(i);    // 某个位置的
            }
        }
        throw new IllegalArgumentException(
    ...
    public ItemViewDelegate getItemViewDelegate(T item, int position)
    {
        ...
        {
            ItemViewDelegate<T> delegate = delegates.valueAt(i);
            if (delegate.isForViewType(item, position))     //ItemViewDelegate 布局管理器
            {
                return delegate;
            }
        }
