...
import com.education.cateye.config.App
 ...
 * @version V_1.0.0
 * @date 2017/8/6
 * @description 海康摄像头播放类
 */
class EzvizCameraPlayer : CameraPlayer, SurfaceHolder.Callback {
    ...
    private var mHandler = object : Handler(Looper.getMainLooper()) {
        override fun handleMessage(msg: Message?) {
            if ((rootView.context as Activity).isFinishing) {
                return
            }
            when (msg?.what) {
    ...
    override fun initWeight(rootView: View) {
        ...
        rootView.title_bar_landscape.addLeftButton(R.drawable.message_back_selector, {
            (rootView.context as Activity).onBackPressed()
        })
    ...
    override fun startPlayer(bundle: Bundle) {
        this.mDeviceInfo = bundle.getParcelable(IntentConsts.EXTRA_DEVICE_INFO)
        this.mCameraInfo = bundle.getParcelable(IntentConsts.EXTRA_CAMERA_INFO)
        if (mStatus == RealPlayStatus.STATUS_START || mStatus == RealPlayStatus.STATUS_PLAY) {
            return
        }

        // 检查网络是否可用
        if (!ConnectionDetector.isNetworkAvailable(App.instance)) {
            // 提示没有连接网络
    ...
    private fun setRealPlayLoadingUI() {
        ...
        DialogManager.getInstance().createLoadingDialog(rootView.context, rootView.resources
